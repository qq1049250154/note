...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj46sujo8wj311o05k42n.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj46xmxi6pj316i0d0wkq.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj7coegsy5j31hg0ciaf5.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlly1gj6q3nb1faj30re0z845m.jpg)一般而言，对于大部分场景来说异步回调的形式就可以了，只有那种需要完全保证不能丢失消息的场景我们做一套完整的解决方案。
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlly1gj6q4nfnlrj30s00d4te6.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlly1gj6q5mfrnmj30ms0okgrf.jpg)

# 那如果消息积压达到磁盘上限，消息被删除了怎么办？

这。。。他妈都删除了我有啥办法啊。。。冷静，再想想。。有了。

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj6nrya1lkj30go0go756.jpg) 最初，我们发送的消息记录是落库保存了的，而转发发送的数据也保存了，那么我们就可以通过这部分数据来找到丢失的那部分数据，再单独跑个脚本重发就可以了。如果转发的程序没有落库，那就和消费方的记录去做对比，只是过程会更艰难一点。
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlly1gj6poslhbwj30zu0h0jw0.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj7ab4c3gbj31460kwn15.jpg)CommitLog文件保存于${Rocket_Home}/store/commitlog目录中，从图中我们可以明显看出来文件名的偏移量，每个文件默认1G，写满后自动生成一个新的文件。

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj7ac6azrzj30yi056q5k.jpg)由于同一个topic的消息并不是连续的存储在commitlog中，消费者如果直接从commitlog获取消息效率非常低，所以通过consumequeue保存commitlog中消息的偏移量的物理地址，这样消费者在消费的时候先从consumequeue中根据偏移量定位到具体的commitlog物理文件，然后根据一定的规则（offset和文件大小取模）在commitlog中快速定位。

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj7ajujmn1j30ys0akn1a.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/007S8ZIlgy1gj7bjmk731j316u0gegpo.jpg)
