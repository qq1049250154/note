...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144659.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144703.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144710.jpg)

### **2.5 动静分离**

为了加快网站的解析速度，可以把动态页面和静态页面由不同的服务器来解析，加快解析速度，降低原来单个服务器的压力。

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144712.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144715.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144719.jpg)

### **3.2 相关文件夹**

然后我们可以使用 `rpm -ql nginx` 来查看 Nginx 被安装到了什么地方，有哪些相关目录，其中位于 `/etc` 目录下的主要是配置文件，还有一些文件见下图：

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144721.jpg)
...
**全局变量名功能**`$host`请求信息中的`Host`，如果请求中没有`Host`行，则等于设置的服务器名，不包含端口`$request_method`客户端请求类型，如`GET`、`POST$remote_addr`客户端的`IP`地址`$args`请求中的参数`$arg_PARAMETERGET`请求中变量名 PARAMETER 参数的值，例如：`$http_user_agent`(Uaer-Agent 值),`$http_referer`...`$content_length`请求头中的`Content-length`字段`$http_user_agent`客户端agent信息`$http_cookie`客户端cookie信息`$remote_addr`客户端的IP地址`$remote_port`客户端的端口`$http_user_agent`客户端agent信息`$server_protocol`请求使用的协议，如`HTTP/1.0`、`HTTP/1.1$server_addr`服务器地址`$server_name`服务器名称`$server_port`服务器的端口号`$scheme`HTTP 方法（如http，https）
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144728.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144731.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144736.png)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144740.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144748.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144753.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144758.jpg)

配置之后 response 的 header 里面多了一个 `Content-Encoding: gzip`，返回信息被压缩了：

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144801.jpg)

注意了，一般 gzip 的配置建议加上 `gzip_min_length 1k`，不加的话：

![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144804.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144809.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144814.jpg)
...
![img](https://typoralim.oss-cn-beijing.aliyuncs.com/img/20210201144819.jpg)
...
https://zhuanlan.zhihu.com/p/144933462