...
![正常缓存处理流程](Redis.assets/缓存的处理流程.png)
...
![try-redis](Redis.assets/try-redis.png)
...
![redis list](Redis.assets/redis-list.png)
...
![img](Redis.assets/redis事件处理器.png)
...
![redis4.0 more thread](Redis.assets/redis4.0-more-thread.png)
...
### [11. Redis是如何判断数据是否过期的呢？](https://snailclimb.gitee.io/javaguide/#/docs/database/Redis/redis-all?id=_11-redis是如何判断数据是否过期的呢？)

Redis 通过一个叫做过期字典（可以看作是hash表）来保存数据过期的时间。过期字典的键指向Redis数据库中的某个key(键)，过期字典的值是一个long long类型的整数，这个整数保存了key所指向的数据库键的过期时间（毫秒精度的UNIX时间戳）。

![redis过期字典](Redis.assets/redis过期时间.png)
...
![redis事务](Redis.assets/redis事务.png)
...
![redis roll back](Redis.assets/redis-rollBack.png)
...
![缓存穿透情况](Redis.assets/缓存穿透情况.png)
...
![image](Redis.assets/加入布隆过滤器后的缓存处理流程.png)
...
### [17. 缓存雪崩](https://snailclimb.gitee.io/javaguide/#/docs/database/Redis/redis-all?id=_17-缓存雪崩)
...
### [19. 参考](https://snailclimb.gitee.io/javaguide/#/docs/database/Redis/redis-all?id=_19-参考)
